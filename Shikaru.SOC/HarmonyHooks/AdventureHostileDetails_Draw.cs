...
namespace Shikaru.SOC.HarmonyHooks
{
            ...
            if (__instance.ScoutingDetailLevel > ScoutingDetailLevel.Far)
            {
                if (__instance.HasThreatLevel)
                {
                    ...
                    if (hasScoutingInfo && flag)
                    {
                        ...
                        drawer.AddText(loc.GetText("Adventure/Scouting/ProvidingSource", new object[] { loc.GetText(__instance.ScoutingProviderKey) }), FontType.LoreMedium, HorizontalAlignment.Center, VerticalAlignment.Middle, FontColor.Granite);
